...
void InjectDLL(HANDLE process, const char* dllPath, const char* dllFunc)
{
	...
	_snprintf_s(injectErrorTitle, MAX_STRING_LENGTH + 1, MAX_STRING_LENGTH, "Fehler");
	...
	_snprintf_s(injectError1, MAX_STRING_LENGTH + 1, MAX_STRING_LENGTH, "Die Inject-DLL '%s' konnte nicht geladen werden.\nDas Programm wird beendet.", injectDLLPath);
	...
	_snprintf_s(injectError2, MAX_STRING_LENGTH + 1, MAX_STRING_LENGTH, "Die Inject-DLL-Einstiegsfunktion '%s' konnte nicht geladen werden.\nDas Programm wird beendet.", injectDLLFunc);
